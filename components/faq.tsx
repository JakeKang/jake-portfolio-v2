...
const renderBoldSegments = (value: string) => {
  ...
  return value.split(/(\*\*[^*]+\*\*)/g).map((segment) => {
    const nextCount = (occurrences.get(segment) ?? 0) + 1;
    occurrences.set(segment, nextCount);
    const key = `${segment}-${nextCount}`;
...
export function FAQ() {
  ...
  return (
    <section
      id='faq'
      className='relative py-20 md:py-28 px-5 md:px-10 lg:px-20 xl:px-28 2xl:px-36 bg-secondary/30 overflow-hidden'>
